...
## LOAD DATA ----
all_wells <- read_rds("input_data_spanish.rds") 
...
types <- c("Activa", "Nuevo", "Tapado", "Fuera de servicio", "Otros") 

# Aggregating Plugged and PluggedOnly here, and putting all other types into "Other"
all_wells <- all_wells %>%
  mutate(WellStatus = ifelse(as.character(all_wells$WellStatus) == "PluggedOnly", 
                             "Tapado", WellStatus)) 

# Update with desired label format on clicking wells
all_wells <- all_wells %>%
  mutate(label = paste0("Este <b>", all_wells$WellTypeLa, "</b> pozo es <b>", 
                        all_wells$WellStatus, "</b>.<br>Fue perforado el <b>", 
                        all_wells$SpudDate, "</b> y es operado por <b>", 
                        all_wells$OperatorNa, "</b>.", 
                        ifelse (!is.na(AbdDate), 
                                paste0(" Fue abandonado el <b>", 
                                       all_wells$AbdDate, "</b>."), "")))
...
wells_active <- all_wells %>%
  filter(WellStatus == "Activa")

wells_new <- all_wells %>%
  filter(WellStatus == "Nuevo")

wells_plugged <- all_wells %>%
  filter(WellStatus == "Tapado") 

wells_idle <- all_wells %>%
  filter(WellStatus == "Fuera de servicio")

wells_other <- all_wells %>%
  filter(!(WellStatus %in% c("Activa", "Nuevo", "Tapado", "Fuera de servicio"))) %>%
  mutate(WellStatus = "Otros") 
...
radii_labels <- c("3200 piés -- propuesta de distancia de amortiguamiento", 
                  "2.5 millas -- efectos observados en la calidad del aire")
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      textInput("text", "Comience a escribir la dirección"), # type address
      uiOutput("geocoder"), # address suggestions
      actionButton("run", "Ejecutar"), # geocodes
      radioButtons("radius", "Seleccionar Radio", # radius selection
...
server <- function(input, output, session) {
  ...
  output$date <- renderText({paste0("Fecha de la última actualización de los datos de los pozos: ", date_of_download)})
  
  ## GEOCODING ----
  # sets up valid address options
  output$geocoder <- renderUI({
    selectInput("address", "Seleccione una dirección válida", 
  ...
  output$map <- renderLeaflet({
    ...
    leaflet(options = leafletOptions(preferCanvas = TRUE)) %>% 
      ...
      addLayersControl( 
        overlayGroups = c(types, "Mostrar escuelas y hospitales cercanos", 
                          "Mostrar límites de los campos petroleros"),
      ...
      htmlwidgets::onRender("
        function() {
            $('.leaflet-control-layers-overlays').prepend('<label style=\"text-align:center\">Seleccionar tipos de pozos a pantalla</label>');
        }
    ") %>%
      setView(lng = val_geo_1(), lat = val_geo_2(), zoom = 12) %>% # center on address/central LA
      # dynamic legend for schools/hospitals
      addLegend(layerId = "legend", group = "Mostrar escuelas y hospitales cercanos", 
                values = c("School", "Hospital"), position = "bottomright",
                labels = c("Escuela", "Hospital"), colors = c("#CE71ED", "#2441C0")) %>%
      # oil field boundaries
      addPolygons(group = "Mostrar límites de los campos petroleros", data = field_boundaries,
                  stroke = F,
                  fillColor = "red",
                  fillOpacity = .3) %>%
      hideGroup("Mostrar escuelas y hospitales cercanos") %>%
      clearGroup("Mostrar escuelas y hospitales cercanos") %>%
      hideGroup("Mostrar límites de los campos petroleros") %>%
  ...
  observe({
    leafletProxy("map") %>%
      clearGroup(group = "Mostrar escuelas y hospitales cercanos") %>%
      addCircles(group = "Mostrar escuelas y hospitales cercanos", data = sens_recept(), 
                 radius = as.numeric(input$radius),
                 stroke = F,
                 fillColor = ifelse(sens_recept()$type == "school", "#CE71ED", "#2441C0"),
                 fillOpacity = .1,
                 lat = as.numeric(sens_recept()$lat), lng = as.numeric(sens_recept()$lon),
                 popup = paste0("<b>", ifelse(sens_recept()$type == "school", paste0("Escuela: "), 
                                              paste0("Hospital: ")), "</b>", 
                                sens_recept()$Name)) %>%
      addCircles(group = "Mostrar escuelas y hospitales cercanos", data = sens_recept(), 
                 radius = 20, stroke = F,
  ...
  observeEvent(val_geo_1(), {
    leafletProxy("map") %>%
      clearMarkers() %>%
      addCircleMarkers(group = "Activa", options = pathOptions(pane = "upper"),
      ...
      addCircleMarkers(group = "Nuevo", options = pathOptions(pane = "upper"),
                       lat = wells_new$Latitude, lng = wells_new$Longitude,
                       clusterOptions = markerClusterOptions(disableClusteringAtZoom = 14),
                       stroke = FALSE, fillOpacity = .8, radius = 3, color = "red",
                       popup = wells_new$label) %>%
      addCircleMarkers(group = "Tapado", options = pathOptions(pane = "upper"),
                       lat = wells_plugged$Latitude, lng = wells_plugged$Longitude,
                       clusterOptions = markerClusterOptions(disableClusteringAtZoom = 14),
                       stroke = FALSE, fillOpacity = .8, radius = 3, color = "red",
                       popup = wells_plugged$label) %>%
      addCircleMarkers(group = "Fuera de servicio", options = pathOptions(pane = "upper"),
                       lat = wells_idle$Latitude, lng = wells_idle$Longitude,
                       clusterOptions = markerClusterOptions(disableClusteringAtZoom = 14),
                       stroke = FALSE, fillOpacity = .8, radius = 3, color = "red",
                       popup = wells_idle$label) %>%
      addCircleMarkers(group = "Otros", options = pathOptions(pane = "upper"),
  ...
  output$numWells <- renderText({paste("<b>", "Número de pozos dentro un radio de: ", numWells(), 
                                       "</b>")})
}
shinyApp(ui, server)
